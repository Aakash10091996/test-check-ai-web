...
import { useGetUIElementsPerTag } from "@/services/typesense";
...
function UiElementCardList() {
  ...
  const { data, isFetching } = useGetUIElementsPerTag(apiRequestBody);
  ...
  useEffect(() => {
    if (activeCategoryItem) {
      setApiRequestBody({
        filter_by: `${searchCategory[0].value} > ${removeHyphen(activeCategoryItem)}`,
        component_pack_name: searchCategory[0].value,
        pageNo: 1,
      });
...
export default UiElementCardList;
